...
static void HuffDump ( LinkType n )
{
	...
	printf("root is %d\n", HuffRoot);
	printf("No.  Sym   Count  Parent   Left  Right  BitNo  Bits\n");
...
static int HuffCompress( FILE *infile, FILE *outfile )
{
	...
	// the root pointer
	fwrite(&HuffRoot, sizeof(HuffCount), 1, outfile);
...
int HuffEncode( FILE *infile, FILE *outfile )
{
	...
	if ( !retval )
	{
		...
		printf("The output file contained %lu header bytes and %lu data bytes\n", HuffBytesHdr, HuffBytesOut);
