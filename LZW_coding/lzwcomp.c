...
CountType LZWTotBytesIn;
CountType LZWTotBytesOut;
CountType LZWCurrBytesIn;
CountType LZWCurrBytesOut;
unsigned LZWBestRatio;
unsigned LZWLastRatio;
...
static void LZWOutFlush( FILE *outfile )
{
	return (0);
}

static void LZWOut( FILE *outfile, CodeType code )
{
...
int LZWEncode(FILE *infile, FILE *outfile)
{
	...
	else
	{
		...
		// make sure we don't sign extend
		currcode &= 0xFF;
				...
				if ( LZWCurrBytesIn >= 10000 )
				{
...
#if defined( HASHSTATS )
					printf("Input: %lu, Output: %lu, Overall Ratio; %lu%%, " , LZWTotBytesOut, LZWTotBytesIn, (LZWTotBytesOut * 100) / LZWTotBytesIn);
					printf("This frame: %u%%\n", ratio);
#endif
					LZWCurrBytesIn = 0;
					LZWCurrBytesOut = 0;
					if ( ratio > LZWBestRatio )
					{
						if ( ratio > 50 && ( ratio > 90 || ratio > LZWLastRatio + 10) )
						{
							...
							for (i = 0; i < DICTIONARY_SIZE; i++)
								dictionary[i].code = UNUSED_CODE;
							SHOWHASH( if ( freecode > FreeMax ) FreeMax = freecode; )
	...
	if ( !retval )
	{
		...
		printf("The output file contained %lu bytes\n", LZWTotBytesOut);
		if ( LZWTotBytesIn != 0 )
			printf("Output file is %lu%% the size of in put file\n", (LZWTotBytesOut * 100) / LZWTotBytesIn);
	}
#endif
#if defined( HASHSTATS )
	{
